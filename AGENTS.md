...
- **Package id is `robosats`.** A single UI daemon (`primary`) running the RoboSats web client, with a hard dependency on `tor` (`kind: 'running'`) for coordinator connectivity — the daemon's `TOR_PROXY_IP`/`TOR_PROXY_PORT` env points at tor's SOCKS proxy (tor's container IP via `sdk.getContainerIp`, since tor's SOCKS is not a StartOS binding). Exports one `ui` interface (id `ui`) on the `ui-multi` host at port 12596; the web health check hits that interface's own LXC-bridge URL at `/selfhosted`.
